...
class TimPass:
    def __init__(self, data: Data):
        ...
        self.var_z: dict[pair, LpVariable] = LpVariable.dicts(
            name='z', indices=data.activities_constrainable.keys(), cat=LpInteger
        )
    ...
    def get_solution(self) -> Solution:
        ...
        timetable = {event_id: result(pi) for event_id, pi in self.var_pi.items()}
        used_edges = [
            (uv, ij)
    ...
    def print_solution(self):
        timetable = {event_id: pi.value() for event_id, pi in self.var_pi.items()}
        used_edges = {
            (uv, ij): p.value()
            for uv, _var in self.var_p.items()
            for ij, p in _var.items()
