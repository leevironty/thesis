...
\thesisabstract{
%     Public transporation are used by many and as such, having good timetables is important. However, optimising the timetables is not trivial, especially if combined with optimising the passenger routes at the same time.
%     In this thesis we propose a graph neural network based heuristic for the routing of passengers. The motivation is, that if we can predict good passenger routings, then the ... to be continued
% }
    Timetable optimization is an important step of a typical public transport planning process. As the routes the passengers choose also affect the optimal time table and vice versa, it is beneficial to jointly optimize the two. However, this quickly becomes computationally intracable. Instead of finding exact solutions, as a heuristical approach we propose to predict the best passenger routings and then to optimize the timetable using established methods. We propose a graph neural network model for predicting the aggregated passenger counts on the event activity network edges. Our approach demonstrates occasional improvements against a shortest path routing baseline. The relationship between the weight prediction loss and the resulting optimality gap indicate, that training the network with a direct regression task may not directly result in improvements of the optimality gap, which is our ultimate objective. Further research to the topic of graph neural networks as a heuristic could explore other training tasks instead of the direct prediction approach. Generating high-quality training datasets is also an area that could be improved.
}
...
In \cref{fig:trivial-shares-in-data} we break down the share of trivial problems by the instance size. We note, that for problems with four or five stops, the instances are almost always (98\% of cases) trivial. The share of zero GNN gap instances is always lower than the share of trivial instances.
